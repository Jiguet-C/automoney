...
const extractBilletsEtPieces = (input) => {
  const billetPieceRegex = new RegExp(
    `\\b(${Object.keys(numberMapping).join(
      "|"
    )})\\s*(de\\s*)?(billet|pièce)s?\\b`,
    "i"
  );
...
// Fonction pour extraire les euros et centimes
const extractEurosEtCentimes = (input) => {
  let euros = 0,
    centimes = 0;

  const euroRegex = new RegExp(
    `\\b(${Object.keys(numberMapping).join("|")})\\s*(euros?|€)\\b`,
    "i"
  );
  const centRegex = new RegExp(
    `\\b(${Object.keys(numberMapping).join("|")})\\s*(centimes?|¢)\\b`,
    "i"
  );
  const euroAndCentRegex = new RegExp(
    `\\b(${Object.keys(numberMapping).join(
      "|"
    )})\\s*(euros?|€)\\s*(et)?\\s*(${Object.keys(numberMapping).join(
      "|"
    )})?\\s*(centimes?|¢)?\\b`,
    "i"
  );
...
const extractBilletsNumbers = (input) => {
  console.log(`Extracting billets from input: "${input}"`);
  ...
  const finalValue =
    detectedNumbers.length > 0 ? detectedNumbers.join(".") : input;
  console.log(`Final Extracted Billet Value: ${finalValue}`);
  return finalValue;
};

// Fonction pour extraire les nombres d'euros
const extractEurosNumbers = (input) => {
  console.log(`Extracting euros from input: "${input}"`);
  const cleanedInput = cleanInput(input);
  const detectedNumbers = [];
  ...
  const finalValue =
    detectedNumbers.length > 1 ? detectedNumbers.join(",") : input;
  console.log(`Final Extracted Euro Value: ${finalValue}`);
  return finalValue;
};
