...
export const calculatePayment = (amountToPay, wallet) => {
  ...
  let amount = parseFloat(
    amountToPay.replace(",", ".").replace(/^(\d+\.?\d{0,2}).*$/, "$1")
  );

  if (isNaN(amount) || amount <= 0) {
    return {
      error: "Montant invalide : Le montant doit être supérieur à zéro",
      solution: null,
    };
  }
  ...
  if (amount > totalWallet) {
    return {
      error: "Erreur : Montant supérieur à ce que contient votre porte-monnaie",
      solution: null,
    };
  }
  ...
  const bestSolution = findBestSolution(
    remainingAmount,
    walletValues,
    walletInCents
  );
  ...
  if (bestSolution) {
    ...
    return {
      error:
        "Erreur : Impossible de régler ce montant avec les pièces/billets disponibles",
      solution: null,
    };
  }
};
